...
THREE.TessellateModifier.prototype.divide = function ( geometry ) {
	var triA,triB;
	var vertices = [];
	var faces = [];
	var faceVertexUvs = [];
	...
	for ( var i = 0, il = geometry.faces.length; i < il; i ++ ) {
		...
		if ( face instanceof THREE.Face4 ) {
			...
			triA = new THREE.Face3( a, b, c, face.normal.clone() );
			triB = new THREE.Face3( a, c, d, face.normal.clone() );
			
			triA.centroid = face.centroid.clone();
			triA.color = face.color.clone();
			
			triB.centroid = face.centroid.clone();
			triB.color = face.color.clone();			
			
			//Normales
			if ( face.vertexNormals.length === 4 ) {
				triA.vertexNormals[ 0 ] = (face.vertexNormals[ 0 ].clone());//a
				triA.vertexNormals[ 1 ] = (face.vertexNormals[ 1 ].clone());//b
				triA.vertexNormals[ 2 ] = (face.vertexNormals[ 2 ].clone());//c
				
				triB.vertexNormals[ 0 ] = (face.vertexNormals[ 0 ].clone());//a
				triB.vertexNormals[ 1 ] = (face.vertexNormals[ 2 ].clone());//c
				triB.vertexNormals[ 2 ] = (face.vertexNormals[ 3 ].clone());//d
			}
			
			//Colores
			if ( face.vertexColors.length === 4 ) {
				triA.vertexColors[ 0 ] = (face.vertexColors[ 0 ].clone());//a
				triA.vertexColors[ 1 ] = (face.vertexColors[ 1 ].clone());//b
				triA.vertexColors[ 2 ] = (face.vertexColors[ 2 ].clone());//c
				
				triB.vertexColors[ 0 ] = (face.vertexColors[ 0 ].clone());//a
				triB.vertexColors[ 1 ] = (face.vertexColors[ 2 ].clone());//c
				triB.vertexColors[ 2 ] = (face.vertexColors[ 3 ].clone());//d
			}

			//Tangentes
			if ( face.vertexTangents.length === 4 ) {
				triA.vertexTangents[ 0 ] = (face.vertexTangents[ 0 ].clone());//a
				triA.vertexTangents[ 1 ] = (face.vertexTangents[ 1 ].clone());//b
				triA.vertexTangents[ 2 ] = (face.vertexTangents[ 2 ].clone());//c
				
				triB.vertexTangents[ 0 ] = (face.vertexTangents[ 0 ].clone());//a
				triB.vertexTangents[ 1 ] = (face.vertexTangents[ 2 ].clone());//c
				triB.vertexTangents[ 2 ] = (face.vertexTangents[ 3 ].clone());//d
			}			
			
			
			faces.push( triA, triB );
			//UVs
			var j, jl, uvA, uvB, uvC, uvD, uvD, uvstriA, uvstriaB;
			for ( j = 0, jl = geometry.faceVertexUvs.length; j < jl; j ++ ) {
				...
				if ( geometry.faceVertexUvs[ j ].length ) {
					...
					uvstriA = [ uvA.clone(), uvB.clone(), uvC.clone() ];
					uvstriB = [ uvA.clone(), uvC.clone(), uvD.clone() ];
					faceVertexUvs[ j ].push( uvstriA, uvstriB );
				}
			}
		}else {
